                ...
                try {
                    ...
                    if (messages.length() != 0) {
                        ...
                        if (messages.length() > 2000) {
                            ...
                            // String messageTooLong = "\n\nThis message has exceeded the discord message
                            // limit (2000 characters) so the rest has been cut out. To see it completely
                            // please check the console itself!";
                        ...
                        try {
                            ...
                            Bukkit.getLogger().severe("[DiscordServerConsole] Invalid channel ID '" + consoleChannelID
                                    + "'! Make sure to put a valid channel ID in the config file! Without this the plugin won't work properly! Disabling plugin...");
                            cancel();
                        }
                        ...
                        if (messagesCont.length() > 0) {
                            ...
                            try {
                                jda.getTextChannelById(consoleChannelID).sendMessage("```" + messagesCont + "```")
                                        .queue();
                            }
                            catch (NumberFormatException numberFormatException) {
                                Bukkit.getLogger().severe("[DiscordServerConsole] Invalid channel ID '"
                                        + consoleChannelID
                                        + "'! Make sure to put a valid channel ID in the config file! Without this the plugin won't work properly! Disabling plugin...");
                                cancel();
                            }
                        ...
                        if (messagesCont2.length() > 0) {
                            ...
                            try {
                                jda.getTextChannelById(consoleChannelID).sendMessage("```" + messagesCont2 + "```")
                                        .queue();
                            }
                            catch (NumberFormatException numberFormatException) {
                                Bukkit.getLogger().severe("[DiscordServerConsole] Invalid channel ID '"
